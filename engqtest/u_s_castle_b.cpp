...
static float vectorA_i;
static U32 vectorA_size;

static vector<capture_path> vectorB; //
static float vectorB_i;
static U32 vectorB_size;

static U32 capture_time = 10; // NLU
...
static bool clearvector;
static bool load;
static bool save;
static bool insertinto;
static bool removefrom;
static bool selectframe;
...
static pbut* REC; // the pbut above it
...
static void updateFrametext()
{
    ...
    if(!vectorAorB){ // A
        sprintf(str,"frame number %6f of %6d",vectorA_i+1, vectorA_size); // string defaults to upper case // %6.3f
        TEXTFRAMESLIDER->settname(str); // change name each frame
    }else // B
        sprintf(str,"frame number %6f of %6d",vectorB_i+1, vectorB_size); // string defaults to upper case // %6.3f
        TEXTFRAMESLIDER->settname(str);
}
...
/// IF VECTOR A
    if(!vectorAorB){ //
/// IF PLAY IS PAUSED
        if (PLAYSLIDER->getidx() == 0){ //  && vectorA_i == 0 ){
            UIstates = IDLE;
            return;
        }
/// IF SOME LEFT TO PLAY
        if(vectorA_i<vectorA_size){
			...
			logger("speed idx = %f\n",speed);
            //if (speed < 0) { /// IF REVERSE
            //    if(vectorA_i > 0)
            //        vectorA_i += speed;
			//} else {
                vectorA_i += speed; /// ELSE FORWARD
			//}
			vectorA_i = range(0.0f,vectorA_i,float(vectorA_size - 1));
            FRAMESLIDER->setminmaxval(1,vectorA_size);
            updateFrametext(); // frame# of total frames, appears on slider text box
            FRAMESLIDER->setidx(vectorA_i+1); //
            updatePlaytext();
        }
/// IF NONE LEFT TO PLAY
        if((vectorA_i+1 == vectorA_size)  || (vectorA_i == 0)){ // && PLAYSLIDER->getidx() < 0)){
            UIstates = IDLE;
//            vectorA_i = 0;
            PLAYSLIDER->setidx(0);
            updateFrametext();
...
/// IF VECTOR B
    else if (vectorAorB){ // vectorB if some left to play
/// IF PLAY IS PAUSED
        if (PLAYSLIDER->getidx() == 0){ //  && vectorA_i == 0 ){
            UIstates = IDLE;
            return;
...
/// IF SOME LEFT TO PLAY
        if(vectorB_i<vectorB_size){
            mainvp.camtrans = vectorB.at(vectorB_i) .cp_camtrans;
            mainvp.camrot = vectorB.at(vectorB_i).cp_camrot;
            if((PLAYSLIDER->getidx() < 0)){ /// IF REVERSE
                if(vectorB_i > 0)
                    --vectorB_i;
            }
            else
                ++vectorB_i;
//            ++vectorB_i;
            FRAMESLIDER->setminmaxval(1,vectorB_size);
            updateFrametext(); // frame# of total frames, appears on slider text box
            FRAMESLIDER->setidx(vectorB_i+1); //
        }
/// IF NONE LEFT TO PLAY
        if((vectorB_i+1 == vectorB_size)  || (vectorB_i <= 0)){ // && PLAYSLIDER->getidx() < 0)){
            UIstates = IDLE;
            vectorB_i = 0;
            PLAYSLIDER->setidx(0);
            updateFrametext();
...
void proc_record(void){
   if(!vectorAorB){ // if vectorA is selected (default )
        cp.cp_camtrans = mainvp.camtrans;
        cp.cp_camrot = mainvp.camrot;
        vectorA.push_back(cp);
    }else{ // if vectorB is selected
        cp.cp_camtrans = mainvp.camtrans;
        cp.cp_camrot = mainvp.camrot;
        vectorB.push_back(cp);
    }
}

void proc_edit(void){
   if(!vectorAorB){ // if vectorA is selected (default )
        FRAMESLIDER->setminmaxval(1,vectorA_size); // range of
        defaultFrametext();
    }else{ // if vectorB is selected
        FRAMESLIDER->setminmaxval(1,vectorB_size); // range of
        defaultFrametext();
    }
...
/// the shape's handles,  in the UI
	...
	for (int i=0;i<sd.num();++i) {
		...
		if (isfileext(str,"txt")) // filter by file extension
	...
	SELECTFILE->setidxc(0); // 0 for files?
	const C8* curfile = SELECTFILE->getidxname(); // display current highl
	if (curfile)
		TEXTSELECTFILE->settname(curfile);
...
//	FFWD=rl->find<pbut>("FFWD");
	...
	PLAYSLIDER->setminmaxval(-10,10); // range of // (-40,40)
	...
	FRAMESLIDER->setminmaxval(1,30000); // range of // (-40,40)
    ...
    pushandsetdir("1mydata");
    if((fpout = fopen("cp1", "rb")) != NULL){ // if file exists
        vectorA.clear();
        capture_path cp;

//        while ( !feof(fpout))
//            fread(&vectorA, sizeof(capture_path), 1, fpout);
...
//    tt->setvis(false);
	tt->trans=pointf3x(-10,10,-10); // -8.071f,8.071f,7.071f*2
	tt->rotvel.x=.003f;
	tt->rotvel.y=.0035f;
...
// cylinders
	cc=buildcylinderc_xz(pointf3x(.25f,1,.75f),"white.pcx",shadernameround, pointf3x(1,0,0), pointf3x(0,1,0), pointf3x(0,0,1));
	cc->trans = pointf3x(0,-.5f,0); // center cylinder
	mycylinder = new tree2("cyl_root");
	mycylinder->linkchild(cc);
...
{
/// MOVIE EDITOR
vectorA_size = vectorA.size(); // check size once each proc cycle
vectorB_size = vectorB.size();
...
// call over or not over proc
    ...
    drawtextque_format_foreback(SMALLFONT,0,315,F32WHITE,F32BLACK,"UI STATE = %d", UIstates);
    shape* focusa = 0;
...
///
        case IDLE:
            updatePlaytext(); // update play speed in play slider text
...
/*
            else if (focusa==FREV){ // fast reverse button, 10 FPS
                UIstates = PLAYIT;
                timespeed = -1/3;
            }
            else if (focusa==REV){ // reverse button, 1 FPS
                UIstates = PLAYIT;
                timespeed = -1/30;
            }
*/
...
/*
            else if (focusa==FWD){ // forward button 1 FPS
                UIstates = PLAYIT;
                timespeed = 1/30;
            }
            else if (focusa==FFWD){ // fast forward button 10 FPS
                UIstates = PLAYIT;
                timespeed = 1/3;
            }
*/
                ...
                updateFrametext(); // update frame number in frame slider and more -- WONT WORK WITHOUT THIS !


            } else if (focusa==VECTOR) { // vector selector
                if(vectorAorB)
                    VECTOR->settname("A");
                else
                    VECTOR->settname("B");
            ...
            } else if (focusa==LOAD) { // vector selector
                if(load)
                    LOAD->settname("LOAD");
                else
                    LOAD->settname("into");
            ...
            } else if (focusa==SAVE) { // vector selector
                if(save)
                    SAVE->settname("SAVE");
                else
                    SAVE->settname("from");
            ...
            } else if (focusa==INSERT) { // vector selector
                UIstates = EDIT;
                if(insertinto)
                    INSERT->settname("INSERT");
                else
                    INSERT->settname("into");
                insertinto = ! insertinto;

            } else if (focusa==REMOVE) { // vector selector
                if(removefrom)
                    REMOVE->settname("REMOVE");
                else
                    REMOVE->settname("from");
            ...
            break; // from previous case
///
            case PLAYIT:
                drawtextque_format_foreback(SMALLFONT,0,330,F32WHITE,F32BLACK,"PLAYIT");
                if (focusa == STOP)
                    UIstates = IDLE;
                break;
///
            case RECORD:
                drawtextque_format_foreback(SMALLFONT,0,410,F32WHITE,F32BLACK,"RECORD");
                if (focusa == STOP)
                    UIstates = IDLE;
...
///
    switch(UIstates){ // OUTSIDE OF THE UI
        case IDLE: // IDLE IS NOT USED HERE
            break;
        case PLAYIT:
            drawtextque_format_foreback(SMALLFONT,0,370,F32WHITE,F32BLACK,"PLAYIT");
            proc_play(); // return a boolean if check
            break;
        case RECORD:
            drawtextque_format_foreback(SMALLFONT,0,440,F32WHITE,F32BLACK,"RECORD");
            proc_record();
            break;
        case EDIT:
            drawtextque_format_foreback(SMALLFONT,0,440,F32WHITE,F32BLACK,"EDIT");
            proc_edit();
            break;
    }
/// MOVIE EDITOR states, ie functionality NLU

//    if (record && (!frev&&!rev&&!play&&!fwd&&!ffwd)){ // or !(frev ||.....)
/*        if(!vectorAorB){
            cp.cp_camtrans = mainvp.camtrans;
            cp.cp_camrot = mainvp.camrot;
            vectorA.push_back(cp);
...
//        vectorA_size=vectorA_size;
//    }

//    if (play  && (!record&&!frev&&!rev&&!fwd&&!ffwd)){ // either vectorA or vectorB

/*        if(!vectorAorB && vectorA_i<vectorA_size){
            if(vectorA_size==0){ // this whole section is not working
//                play = !play;
                PLAY->settname("play");
                return;
            }
            ...
            ++vectorA_i;
            updateFrametext();
            if(vectorA_i+1 == vectorA_size){
                play = !play;
                vectorA_i = 0; // vectorA.begin();
                PLAY->settname("play");
                updateFrametext();
            }
        }else if(vectorAorB &&vectorB_i<vectorB_size){
            if(vectorB_size==0){ // not working
//                play = !play;
                PLAY->settname("play");
                return;
            }
            ...
            ++vectorB_i;
            updateFrametext();
             if(vectorB_i+1 == vectorB_size){
                play = !play;
                vectorB_i = 0; // vectorB.begin();
                PLAY->settname("play");
                updateFrametext();
             }
        }*/
...
//	drawtextque_format_foreback(SMALLFONT,0,10,F32WHITE,F32BLACK,"'p' to change path, curpath %d/%d, curtime = %f",curpath+1,npathfunc,curtime); // 2d print in a 3d world
	...
	drawtextque_format_foreback(SMALLFONT,0,210,F32WHITE,F32BLACK,"vectorA_size = %d, vectorB_size = %d",vectorA_size, vectorB_size);
	drawtextque_format_foreback(SMALLFONT,0,250,F32WHITE,F32BLACK,"A frame number = %f, B frame number = %d",vectorA_i+1,vectorB_i+1);
	drawtextque_format_foreback(SMALLFONT,100,290,F32WHITE,F32BLACK,"slider frame number = %f",FRAMESLIDER->getidx());
...
{
/// MOVIE EDITOR
    pushandsetdir("1mydata"); // not working, stays in data/
//    if((fpout = fopen("cp1", "rb")) == NULL){ // if this file does not exist, write to it
//        fclose(fpout);
        fpout = fopen("cp1", "wb");
        for (vectorA_i=0; vectorA_i<vectorA_size; vectorA_i++){
            fwrite(&vectorA[vectorA_i], sizeof(capture_path), 1, fpout);  // &vectorA is wrong. use & on arrays only, not vectors
        }
        fclose(fpout);
//    }
    popdir(); // unknown effect
// from testres3d
