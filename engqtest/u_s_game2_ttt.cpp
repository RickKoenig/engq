...
class board_ttt : public board {
public:
	board_ttt(const board& ba) : board(ba) {}
	board_ttt(string levelnamea) : board(levelnamea)
	{
...
gamestatus board_ttt::getresult() const
{
...
// return new piece
S32 board_ttt::findcloseidx(bool f) const
{
	return NEWPIECEIDX;
}

void board_ttt::movepiece(S32 stidx,const pointi2b8& endpos)
{
...
game<board_ttt>* agame;
S32 curlevel;
string curlevelname;
script* sc;

#ifdef USENAMESPACE
} // end namespace game2_ttt
using namespace game2_ttt;
#endif

void game2_tttinit()
{
	video_setupwindow(800,600);
	pushandsetdir("game2_ttt");
	sc = game<board_ttt>::getlevels();
	S32 i;
	for (i=0;i<sc->num();++i)
	...
	agame = new game<board_ttt>(curlevelname);
	agame->studygames();
}
