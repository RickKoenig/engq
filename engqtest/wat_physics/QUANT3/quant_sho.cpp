...
#define MAXENERGY 57 // 0 to 31 // actually not energy but different eigenstates, before starts at 1, after starts at 0
...
vector<float> sintbtri; // float sintb[TRIGSIZE]; // trig table
...
float sinttri(S32 t)
{
	return sintbtri[(TRIGSIZE-1)&t];
}

float costtri(S32 t)
{
	return sintbtri[(TRIGSIZE-1)&(t+TRIGSIZE/4)];
}
...
S32 qnumber = 0;
const U32 ESsize = ENERGYARRSIZE;
QF hp[ESsize][ESsize];
void calcherm()
{
	S32 i,j;
...
float timeind(S32 x,S32 n) {
	S32 rx=lshift((n+1)*x,LTRIGSIZE-LSPACESIZE-1);
//	return sint(rx);
//	return sinttri(rx);
	float xf = 20.0f*x/SPACESIZE - 10.0f;
	return eigen_n(xf,n);
}
...
// not so important
		...
		for (x=0;x<SPACESIZE;x++)
			for (n=0;n<ENERGYARRSIZE;n++) {
				angsx[x][n]=timeind(x,n)*ak[n];
			}
		for (t=0;t<TIMESIZE;t++) {
			for (n=0;n<ENERGYARRSIZE;n++) {
				// rt=lshift(n*n*t + phk[n],LTRIGSIZE-LTIMESIZE); // infinite square well
				rt = lshift((1 + 2*n)*t + phk[n],LTRIGSIZE-LTIMESIZE); // SHO
...
void update_energy_list()
{
	...
	lenergies->clear();
	for (i=0;i<ENERGYARRSIZE;++i) { // 1 to 31, 31 in all
		if (!ak[i])
			phk[i]=0;
//		if (ak[i]) {
...
// init trig tables
	...
	sintbtri.assign(TRIGSIZE,0);
#endif
	for (i=0;i<TRIGSIZE;i++) {
		sintb[i]=sinf(i*TWOPI/TRIGSIZE);
		if (i<TRIGSIZE/4) {
			sintbtri[i] = i*4.0f/TRIGSIZE;
		} else if (i<3*TRIGSIZE/4) {
			sintbtri[i] = -i*4.0f/TRIGSIZE + 2.0f;
		} else {
			sintbtri[i] = i*4.0f/TRIGSIZE - 4.0f;
		}
	}
// init energies
#ifdef USEVECTOR
