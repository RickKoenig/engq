...
pbut* pbutpost,*pbutabort,*pbutname2ip,*pbutip2name;
edit* editip,*editname,*editfile,*editpost;
...
S32 wtpos,rdpos;
enum bus {BUSY_NONE,BUSY_NAME2IP,BUSY_IP2NAME,BUSY_POST};
bus busy;

C8 post_url[]="cgi.www.emccsoft.com"; // test http POST server, hope it works
//C8 post_url[]="cgi.www.emccsoft.com/cgi-bin/www.emccsoft.com/post.pl"; // test http POST server, hope it works
//C8 post_url[]="api.test.workatplay.com"; // was never up
//C8 post_url[]="test.api.hwp.workatplay.com"; // was up now down
...
void post_init()
{
	...
	pbutpost=rl->find<pbut>("PBUTPOST");
	...
	editpost=rl->find<edit>("EDITPOST");
	...
	if (focus == pbutquit) {
		...
		if (ret==1) {
			...
			if (!cly) {
				...
				textstatus->settname("posting");
				busy=BUSY_POST;
				con32_printf(con,"=======================================\n");
				httpstr2 = "POST ";
				httpstr2 += editfile->gettname();
				httpstr2 += " HTTP/1.0\r\n";
				S32 conlen=strlen(editpost->gettname());
				C8 clstr[100];
				sprintf(clstr,"Content-Length: %d\r\n\r\n",conlen);
				httpstr2 += clstr;
				httpstr2 += editpost->gettname();
				httpstr2 += "\r\n\r\n";
...
//S32 httpstrlen;
/*C8 post_testtext[]=
	"POST /cgi-bin/www.emccsoft.com/post.pl HTTP/1.0\n"
