...
// fog test
	...
	videoinfodx9.fog.mode=2;
	videoinfodx9.fog.color=pointf3x(.5f,.5f,.5f);
	videoinfodx9.fog.start=1.0f;
	videoinfodx9.fog.end=100.0f;
	videoinfodx9.fog.density=.03225f;
// end fog test
...
//	if (!dosky)
//		tt->mod->mats[0].msflags &= ~SMAT_ISSKY;
	popdir();
...
//		if (!dosky)
//			skyboxtree->mod->mats[0].msflags &= ~SMAT_ISSKY;
		popdir();
//	tt=buildskybox(pointf3x(100,100,100),"cubicmap.jpg","tex");
//		skyboxtree->mod->mats[0].msflags|=SMAT_HASNOFOG;
