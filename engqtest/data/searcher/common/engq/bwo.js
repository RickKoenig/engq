...
/*			for (i=0;i<oio.faces.length;++i) {
				if (i >= head)
					break;
				//logger("   DATA ARRAY[" + i + "]: FACE with matidx " + oio.faces[i].idx[0] + " " + oio.faces[i].idx[1] + " " + oio.faces[i].idx[2] + " " + oio.faces[i].matidx + "\n");
			} */
			break;
...
//	return oio;
	...
	for (i=0;i<oio.mats.length;++i) {
		...
		switch(dolightmap) {
		case 0:
			ret.addmat("tex",mat.dtex,mat.fs,mat.vs);
			break;
		case 1:
			if (candolightmap) {
		...
		case 2:
			if (candolightmap) {
				ret.addmat("tex",globallmname,mat.fs,mat.vs); // switch over to alt texture
			} else {
				ret.addmat("tex",mat.dtex,mat.fs,mat.vs);
			}
			break;
