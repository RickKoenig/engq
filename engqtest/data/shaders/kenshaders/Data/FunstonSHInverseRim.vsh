...
// 19
	mul		rim1.xyz,finalRimFactor,RimColor
	mov		rim1.w,ZEROV.w
// 20
	sub		finalRimFactor,ONEV.x,finalRimFactor
	mad		oColor,tcolor,finalRimFactor,rim1
...
//	add		rim1.xyz,rim1,tcolor
//	mov		oColor,rim1
    end	// output to all output registers before calling 'end'
endmain:
/*
const char FunstonSHInverseRimVertex[] =
	"#version 110\n"
...
const char FunstonSHInverseRimFrag[] =
	"#version 110\n"
...
// 18
	"    float finalRimFactor = max(smoothstep(1.0 - RimMaximum, 1.0, viewDot), 0.0) ;\n"
	//"    gl_FragColor = vec4(finalRimFactor, finalRimFactor, finalRimFactor, 0.0) + (g_Color * texture2D(tex1, vec2(g_UV.x, 1.0 - g_UV.y)));\n"
// 19
	"    gl_FragColor = vec4((finalRimFactor * RimColor), 0.0) + 
// 20
		(1.0 - finalRimFactor) * g_Color * texture2D(tex1, vec2(g_UV.x, 1.0 - g_UV.y));\n"
