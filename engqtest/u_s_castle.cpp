...
/// SOUND
    ...
    logger ("fileexist mammothTrumpet = %d\n", fileexist("mammothTrumpet"));
    twh = wave_load("mammothTrumpet.ogg",0);
    logger ("mammothTrumpet ptr = %p\n", twh);
    popdir();
//    playasound(twh);
...
// with spotlight
	...
	amb->lightcolor = F32WHITE; // LIGHTBLUE
...
//	dir->flags |= TF_ISLIGHT;
	...
	dir->flags |= TF_ISLIGHT;
	dir->lightcolor = F32BLACK; // LIGHTGREEN
	me->linkchild(dir);
	addlighttolist(dir);
...
// tori
    ring=buildtorus_xz(1,.1f,"white.pcx",shadernameround, 30, 30);
    ring->name="ring";
...
/// ANIMATE TIGER
    logger ("tarpits ptr = %p\n", tarpits);
    tarpits = new tree2("tarpits.BWS");
    if(!tarpits)
        errorexit("cant find tarpits.BWS");
    logger ("tarpits ptr = %p\n", tarpits);

    C8 fnamem[MAX_PATH] = "mountain.bwo";
//#define USENEWDUP
#ifdef USENEWDUP
	tree2* mount = tarpits->find(fnamem)->newdup(); // a name with a .bwo or .bws will load and build them
	mount->buildo2p = O2P_FROMTRANSROTSCALE; // don't use o2p none, use something that will read trans rot and scale instead
#else
	tree2* mount = new tree2(fnamem); // a name with a .bwo or .bws will load and build them
#endif
	roottree->linkchild(mount);
    mount->scale = pointf3x(.05f,.05f,.05f);
    mount->rot = pointf3x(0,0,0);
    //mount->rot = pointf3x(0,PI+PI/2,0);
    mount->trans = pointf3x(0,0,0);
#if 1
	tpbwofile = tarpits->find("Wooly_null.bwo");
	//tpbwofile = tarpits->find("wooly.bwo");
    if(!tpbwofile)
        errorexit("cant find tpbwofile");
    logger ("tpbwofile ptr = %p\n", tpbwofile);
    wolly_nulldup = tpbwofile->newdup();
	tree2* woolly_tmp = new tree2("intermediate");
	woolly_tmp->trans = pointf3x(855,-5,-176);
	woolly_tmp->linkchild(wolly_nulldup);
	roottree->linkchild(woolly_tmp);

	//wolly_nulldup->trans = pointf3x(); /// CRASH !!!
	//wolly_nulldup->buildo2p = O2P_FROMTRANSROTSCALE;
	seq_start(wolly_nulldup);
//	roottree->linkchild(wolly_nulldup);
#endif
//	wolly_nulldup->flags |= TF_DONTDRAWC;
//    wolly_nulldup->scale = pointf3x(2,2,2);
//    wolly_nulldup->rot = pointf3x(0,PI+PI/2,PI); // PI+PI/2
/*    wolly_nulldup->trans = pointf3x(30,10,-30);
*/

/*    tpbwofile = tarpits->find("Volcanos.bwo");
    volcanos = tpbwofile->newdup();
	volcanos->trans = pointf3x();
//	volcanos->buildo2p = O2P_FROMTRANSROTSCALE;
	roottree->linkchild(volcanos);
    volcanos->scale = pointf3x(1,1,1);
    volcanos->rot = pointf3x(0,0,0);
    volcanos->trans = pointf3x(0,10,-50);
//*/
	popdir();
...
/// FORTPOINT
    ...
    logger ("fortpoint ptr = %p\n", fortpoint);
/// bunk bed
    fpbwofile = fortpoint->find("Bunk_bed.bwo");
    logger ("fpbwofile ptr = %p\n", fpbwofile);
    bunkbeddup = fpbwofile->newdup();
	bunkbeddup->trans = pointf3x();
	bunkbeddup->buildo2p = O2P_FROMTRANSROTSCALE;
	roottree->linkchild(bunkbeddup);
//	bunkbeddup->flags |= TF_DONTDRAWC;
    bunkbeddup->scale = pointf3x(2,2,2);
    bunkbeddup->rot = pointf3x(0,PI+PI/2,0); // PI+PI/2
    bunkbeddup->trans = pointf3x(21,2,9);
/// cannons
    fpbwofile = fortpoint->find("CannonT_LP01.bwo");
    logger ("fpbwofile ptr = %p\n", fpbwofile);
    cannon1 = fpbwofile->newdup();
	cannon1->trans = pointf3x();
	cannon1->buildo2p = O2P_FROMTRANSROTSCALE;
	roottree->linkchild(cannon1);
//	cannon1->flags |= TF_DONTDRAWC;
    cannon1->scale = pointf3x(2,2,2);
    cannon1->rot = pointf3x(0,0,0); // pointf3x(0,0,0); // PI+PI/2
//    cannon1->rotvel.y = .01; // pointf3x(0,0,0); // PI+PI/2
    cannon1->trans = pointf3x(20,11.6,6);

    fpbwofile = fortpoint->find("CannonT_LP.bwo");
    logger ("fpbwofile ptr = %p\n", fpbwofile);
    cannon2 = fpbwofile->newdup();
	cannon2->trans = pointf3x();
	cannon2->buildo2p = O2P_FROMTRANSROTSCALE;
	roottree->linkchild(cannon2);
//	cannon2->flags |= TF_DONTDRAWC;
    cannon2->scale = pointf3x(2,2,2);
    cannon2->rot = pointf3x(0,PI/2,0); // pointf3x(0,0,0); // PI+PI/2
//    cannon2->rotvel.y = .01; // pointf3x(0,0,0); // PI+PI/2
    cannon2->trans = pointf3x(40,11.6,6);

    fpbwofile = fortpoint->find("cannonballs.bwo");
    logger ("fpbwofile ptr = %p\n", fpbwofile);
    cannonballs = fpbwofile->newdup();
	cannonballs->trans = pointf3x();
	cannonballs->buildo2p = O2P_FROMTRANSROTSCALE;
	roottree->linkchild(cannonballs);
    cannonballs->scale = pointf3x(2,2,2);
    cannonballs->rot = pointf3x(0,PI/2,0); // pointf3x(0,0,0); // PI+PI/2
    cannonballs->trans = pointf3x(25,11.6,6);

    cannonballs1 = fpbwofile->newdup();
	cannonballs1->trans = pointf3x();
	cannonballs1->buildo2p = O2P_FROMTRANSROTSCALE;
	roottree->linkchild(cannonballs1);
    cannonballs1->scale = pointf3x(2,2,2);
    cannonballs1->rot = pointf3x(0,PI/2,0); // pointf3x(0,0,0); // PI+PI/2
    cannonballs1->trans = pointf3x(35,11.6,6);

    fpbwofile = fortpoint->find("Caution_sign.bwo");
    logger ("fpbwofile ptr = %p\n", fpbwofile);
    cautionsign = fpbwofile->newdup();
	cautionsign->trans = pointf3x();
	cautionsign->buildo2p = O2P_FROMTRANSROTSCALE;
	roottree->linkchild(cautionsign);
    cautionsign->scale = pointf3x(2,2,2);
    cautionsign->rot = pointf3x(0,PI/2,0);
    cautionsign->trans = pointf3x(30,11.6,6);

    fpbwofile = fortpoint->find("benches2.bwo");
    logger ("fpbwofile ptr = %p\n", fpbwofile);
    benches = fpbwofile->newdup();
	benches->trans = pointf3x();
	benches->buildo2p = O2P_FROMTRANSROTSCALE;
	roottree->linkchild(benches);
    benches->scale = pointf3x(4,4,4);
    benches->rot = pointf3x(0,PI/2,0);
    benches->trans = pointf3x(30,12,58);

    fpbwofile = fortpoint->find("SUV.bwo");
    logger ("fpbwofile ptr = %p\n", fpbwofile);
    SUV = fpbwofile->newdup();
	SUV->trans = pointf3x();
	SUV->buildo2p = O2P_FROMTRANSROTSCALE;
	roottree->linkchild(SUV);
    SUV->scale = pointf3x(2,2,2);
    SUV->rot = pointf3x(0,0,0);
    SUV->trans = pointf3x(30,2,-20);

    fpbwofile = fortpoint->find("SFPD_car.bwo");
    logger ("fpbwofile ptr = %p\n", fpbwofile);
    SFPDcar = fpbwofile->newdup();
	SFPDcar->trans = pointf3x();
	SFPDcar->buildo2p = O2P_FROMTRANSROTSCALE;
	roottree->linkchild(SFPDcar);
    SFPDcar->scale = pointf3x(2,2,2);
    SFPDcar->rot = pointf3x(0,0,0);
    SFPDcar->trans = pointf3x(10,2,-20);

    fpbwofile = fortpoint->find("Restrooms.bwo");
    logger ("fpbwofile ptr = %p\n", fpbwofile);
    restrooms = fpbwofile->newdup();
	restrooms->trans = pointf3x();
	restrooms->buildo2p = O2P_FROMTRANSROTSCALE;
	roottree->linkchild(restrooms);
    restrooms->scale = pointf3x(1,1,1);
    restrooms->rot = pointf3x(0,0,0);
    restrooms->trans = pointf3x(-10,3.5,-20);

    fpbwofile = fortpoint->find("Roadblock.bwo"); // cement barriers
    logger ("fpbwofile ptr = %p\n", fpbwofile);
    roadblock = fpbwofile->newdup();
	roadblock->trans = pointf3x();
	roadblock->buildo2p = O2P_FROMTRANSROTSCALE;
	roottree->linkchild(roadblock);
    roadblock->scale = pointf3x(1,1,1);
    roadblock->rot = pointf3x(0,0,0);
    roadblock->trans = pointf3x(-10,0,-30);

    fpbwofile = fortpoint->find("Bed.bwo"); //
    logger ("fpbwofile ptr = %p\n", fpbwofile);
    bed = fpbwofile->newdup();
	bed->trans = pointf3x();
	bed->buildo2p = O2P_FROMTRANSROTSCALE;
	roottree->linkchild(bed);
    bed->scale = pointf3x(1,1,1);
    bed->rot = pointf3x(0,0,0);
    bed->trans = pointf3x(0,10,-30);

    fpbwofile = fortpoint->find("terrain.bwo"); //
    logger ("fpbwofile ptr = %p\n", fpbwofile);
    terrain = fpbwofile->newdup();
	terrain->trans = pointf3x();
	terrain->buildo2p = O2P_FROMTRANSROTSCALE;
	roottree->linkchild(terrain);
    terrain->scale = pointf3x(1,1,1);
    terrain->rot = pointf3x(0,0,0);
    terrain->trans = pointf3x(0,100,-200);
    ///*/
	popdir();
