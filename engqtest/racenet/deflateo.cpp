...
static struct hufftab litlenhuff[256],disthuff[256];
...
//					logger("distlen for %3d: is %d\n",litptr-nlit,litcode);
	...
	genhuff(litlenhuff,8,litdistlens,nlit);
	genhuff(disthuff,8,litdistlens+nlit,ndist);
/*	while(cnt--) {
...
//	logger("decoding %d pixels....\n",prod);
	...
	while(1) {
		...
		unsigned int ll,ld;
		litlencode=gethuff(&compdata,&bitpos,litlenhuff,8);
//		logger("pixcnt = %d\n",pc);
		if (litlencode==256) {
			if (pc != prod)
...
//			logger("lit len %d, ",ll);
			distcode=gethuff(&compdata,&bitpos,disthuff,8);
