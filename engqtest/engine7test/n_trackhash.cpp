...
#if 0
void gettrackhash(unsigned char *trk,int len,unsigned char *hash)
{
	int i;
	for (i=0;i<NUMHASHBYTES;i++)
		hash[i]=0;
	for (i=0;i<len;i++)
		hash[i&(NUMHASHBYTES-1)]+=(i+1)*trk[i];
}
#endif
#if 0
 ...
 */
extern FILE* fopen2(const char* name,const char* mode);
static void MDFile (
char *filename
)
{
  ...
  if ((file = fopen2 (filename, "rb")) == NULL)
