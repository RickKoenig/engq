...
#endif
const char *savenewconstrack(const char *name,const newtrack *td,int u)
{
...
#define NUMDT 3
static C8* dumtexn[NUMDT]={	"rvrbnkshrubs.dds","track02b.dds","grnd02m.dds"};
static modelb* dumm;
...
//				setresname(rlnn,EDITNAME,uniquetrackname(getresname(rl,TEXTTRACK)));
				EDITNAME->settname(uniquetrackname(TEXTTRACK->gettname()));
...
//				setresname(rlnn,EDITNAME,uniquetrackname(getresname(rlnn,EDITNAME)));
				EDITNAME->settname(uniquetrackname(EDITNAME->gettname()));
				consmode=MODE_NORMAL;
				focus=0;
