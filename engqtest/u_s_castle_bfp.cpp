...
vector<capture_path>* sourcevector = &vectorB; // usually from B, but could be reversed
...
/// IF SOME LEFT TO PLAY
    if(*curvectori < curvector->size()){
        mainvp.camtrans = curvector->at(*curvectori).cp_camtrans; // curvector-> // vectorA.
        mainvp.camrot = curvector->at(*curvectori).cp_camrot;
        speed = PLAYSLIDER->getidx() * .1f;
        *curvectori += speed; // ???
...
//            FRAMESLIDER->setminmaxval(1,vectorA.size()); // why is this here ???
        updateFrameSlidertext(); // frame# of total frames, appears on slider text box
...
// sound
	...
	logger("fileexist mammothTrumpet.ogg = %d\n",fileexist("mammothTrumpet.ogg"));
...
//	}
    ...
    ///
            case IDLE: // check all the buttons and sliders
                updatePlaySlidertext(); // update play speed in play slider text
                updateFrameSlidertext();
                if (focusa==QUIT){ // cancel button
                    ...
                    *curvectori = FRAMESLIDER->getidx()-1;
                    ...
                    loadfilename = SELECTFILE->getidxname(); // display current highlighted file on text bar AND save it to loadfilename
                    if (loadfilename)
                        TEXTSELECTFILE->settname(loadfilename);
                    pushandsetdir("1mydata");
                    ...
                    tempconstcharptr = SAVEFILENAME->gettname(); // |3472|error: invalid conversion from 'const C8* {aka const char*}' to 'char*' [-fpermissive]|
					...
					tempcharptr[i] = '\0';
                    strcat(tempcharptr, ".me"); // add extension
                    savefilename = tempcharptr; // init the final const char* and send it to fopen(savefilename, "wb") in castle....
...
//        fclose(fpout);
    fpout = fopen("cp1", "wb");
    for (vectorA_i=0; vectorA_i<vectorA.size(); vectorA_i++){
