//#define BERK // if defined, then use 'l_socker_berk.cpp' instead of 'l_socker.cpp' try to simulate Berkeley sockeets

// trick to avoid long compile times, this api should be close enought to 'l_socker.h'
#if 0
// win socker api
...
// write functions
S32 tcpgetwbuffleft(struct socker*);
...
int tcpwritetoken8(struct socker *,int id,char *tok,int len); // ret >0 for valid write, 0 full, -1 err
int tcpreadtoken32(struct socker *,int *id,char *tok,int len); // ret >0 for valid tok, 0 not yet, -1 err
int tcpwritetoken32(struct socker *,int id,char *tok,int len); // ret >0 for valid write, 0 full, -1 err
...
S32 udpputbuffdata(struct socker*,C8* buff,S32 len,U32 toip,U32 toport); // buff ----> app 
