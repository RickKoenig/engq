...
shared float g_uvoffsetu;					// texvels u
shared float g_uvoffsetv;					// texvels v
...
// to texvels
    Output.TextureUV.x = vTexCoord0.x + g_uvoffsetu; 
    Output.TextureUV.y = vTexCoord0.y + g_uvoffsetv; 
// copy normals over to world space
	Output.normsw = mul(norm,g_mWorld);
// copy normals over to object space
	Output.normso = norm;
// get half vectors over
    float3 eyedir = vPos.xyz - g_eyeposobj;
